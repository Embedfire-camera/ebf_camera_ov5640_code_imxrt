...
{
	SCCB_WriteMultiRegs(LPI2C1, OV5640_SCCB_ADDR, kSCCB_RegAddr16Bit,0x8000,pBuffer,BufferSize);
 ...
 uint8_t OV5640_FOCUS_AD5820_Init(void)
{
    ...
    OV5640_WriteReg(LPI2C1,0x3000, 0x20);
...
//  写入自动对焦固件 Brust mode
    ...
    OV5640_WriteReg(LPI2C1,0x3022, 0x00);
    OV5640_WriteReg(LPI2C1,0x3023, 0x00);
    OV5640_WriteReg(LPI2C1,0x3024, 0x00);
    OV5640_WriteReg(LPI2C1,0x3025, 0x00);
    OV5640_WriteReg(LPI2C1,0x3026, 0x00);
    OV5640_WriteReg(LPI2C1,0x3027, 0x00);
    OV5640_WriteReg(LPI2C1,0x3028, 0x00);
    OV5640_WriteReg(LPI2C1,0x3029, 0xFF);
    OV5640_WriteReg(LPI2C1,0x3000, 0x00);
    OV5640_WriteReg(LPI2C1,0x3004, 0xFF);
    OV5640_WriteReg(LPI2C1,0x0000, 0x00);
    OV5640_WriteReg(LPI2C1,0x0000, 0x00);
    OV5640_WriteReg(LPI2C1,0x0000, 0x00);
    OV5640_WriteReg(LPI2C1,0x0000, 0x00);
		
    do {
        OV5640_ReadReg(LPI2C1,0x3029,&state);
...
uint8_t OV5640_FOCUS_AD5820_Constant_Focus(void)
{
    ...
    OV5640_WriteReg(LPI2C1,0x3023,0x01);
    OV5640_WriteReg(LPI2C1,0x3022,0x04);

    iteration = 5000;
    do {
    	 OV5640_ReadReg(LPI2C1,0x3023,&state);
        if (iteration-- == 0)
        {
...
//4.send single focus mode command to firmware
    OV5640_WriteReg(LPI2C1,0x3023,0x01);
    OV5640_WriteReg(LPI2C1,0x3022,0x03);

    PRINTF("after single focus  \n");
		/*	5.在sigle focus命令之后，检查sta_focus直到s_focus 0x10	*/
    iteration = 1000;  
    do{
        OV5640_ReadReg(LPI2C1,0x3023,&state);
...
uint8_t OV5640_FOCUS_AD5820_Pause_Focus(void)
{
    ...
    uint32_t iteration = 300;
		/* 向固件发送空闲命令 */
    OV5640_WriteReg(LPI2C1,0x3023,0x01);
    OV5640_WriteReg(LPI2C1,0x3022,0x06);
		/* 变量赋值 */
    iteration = 100;  
    do{
				/*	写入数据	*/
...
uint8_t OV5640_FOCUS_AD5820_Cancel_Focus(void)
{
    uint8_t state = 0x8F;
    uint32_t iteration = 300;
    PRINTF("OV5640_FOCUS_AD5820_Cancel_Focus\n");
		/* 向固件发送空闲命令 */
    OV5640_WriteReg(LPI2C1,0x3023,0x01);
    OV5640_WriteReg(LPI2C1,0x3022,0x08);
		/* 变量赋值 */
    iteration = 100;  
    do{
        OV5640_ReadReg(LPI2C1,0x3023,&state);
...
void OV5640_FOCUS_AD5820_Check_MCU(void)
{
    ...
    OV5640_ReadReg(LPI2C1,0x3000,&check[0]);
    OV5640_ReadReg(LPI2C1,0x3004,&check[1]);
    OV5640_ReadReg(LPI2C1,0x3f00,&check[2]);	
    OV5640_ReadReg(LPI2C1,0x3001,&check[3]);
    OV5640_ReadReg(LPI2C1,0x3005,&check[4]);
    OV5640_ReadReg(LPI2C1,0x3018,&check[5]);
    OV5640_ReadReg(LPI2C1,0x301e,&check[6]);
    OV5640_ReadReg(LPI2C1,0x301b,&check[7]);
    OV5640_ReadReg(LPI2C1,0x3042,&check[8]);
    OV5640_ReadReg(LPI2C1,0x3018,&check[9]);
    OV5640_ReadReg(LPI2C1,0x301e,&check[10]);
    OV5640_ReadReg(LPI2C1,0x301b,&check[11]);
    OV5640_ReadReg(LPI2C1,0x3042,&check[12]);
    for(i = 0; i < 13; i++)
    PRINTF("check[%d]=0x%x\n", i, check[i]);
}
